...
//import 'package:eadega/src/providers/empty_state_provider.dart';
...
class _OrderCreatePageOtState extends State<OrderCreatePageOt> {
      ...
      appBar: AppBar(
        ...
        actions: <Widget>[
          FlatButton(
            ...
            onPressed: (!_enableButtonSave)
                ? null
                : () {
                    var validState = detalles.firstWhere(
                        (e) => e.product.idUbicacion == null,
                        orElse: () => null);
  ...
  void onAddForm() {
    getProduct(_searchProduct.text).then((value) {
      setState(() {
        ...
        detalles.add(OrderFormOt(
            product: _product, index: _count, onDelete: () => null));
      });
    });
  }
  ...
  void _reprintFormsSearch(ProductModel product) {
    print(product.glosa);
    if (product == null) {
      var baseDialog = BaseAlertDialog(
          title: "Error",
      ...
      detalles.forEach((element) {
        ...
        if (double.parse(element.product.cantidad) > cant) {
          var baseDialog = BaseAlertDialog(
            title: "Error",
            content:
                "La cantidad del producto no es suficiente, ¿desea generar otra linea de detalle?",
            yesOnPressed: () {
              setState(() {
                _count++;
                ProductModel _product = new ProductModel(
                ...
                if (productCode == product.producto) {
                  setState(() {
                    ...
                    element.state.unidadMedidaEditor.text =
                        product.unidadMedida;
                    detalles.elementAt(element.index - 1).product.idUbicacion =
                        product.idUbicacion;
                    detalles.elementAt(element.index - 1).product.unidadMedida =
                        product.unidadMedida;
                  });
                }
              });
          ...
          if (productCode == product.producto) {
            //element.state.ubicacionEditor.text = product.idUbicacion;
            //element.state.unidadMedidaEditor.text =
            //    product.unidadMedida.toString();
            detalles.elementAt(element.index - 1).product.idUbicacion =
                product.idUbicacion;
            detalles.elementAt(element.index - 1).product.unidadMedida =
                product.unidadMedida;

            //detalles.elementAt(element.index - 1).state.ubicacionEditor.text =
            //product.idUbicacion;
            detalles.elementAt(element.index - 1).backgroundColor =
                Colors.green.shade300;

            var det = detalles.elementAt(element.index - 1);
            detalles.removeAt(element.index - 1);
            setState(() {
              //_count++;
              detalles.add(OrderFormOt(
                product: det.product,

...
class _ReceptionQuantityState extends State<ReceptionQuantity> {
  ...
  List<Detail> _castClassPurchase(List<dynamic> obj){
    ...
    obj.forEach((element) {
      ...
      notifications.add(
        new NotificationSetting(title: d.glosa, subtitle: 'Cantidad', quantity: d.cantidad, indexItem: indexVal)
      );
      lstDetail.add(d);
      indexVal++;
    });
  ...
  _row(NotificationSetting element) {
      return Row(
        children: [
          ...
          Expanded(
            child: TextFormField(
              initialValue: (element != null)?double.parse(element.quantity).toString():0.toString(),
              onChanged: (value){
